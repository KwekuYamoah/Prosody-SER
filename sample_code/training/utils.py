...
def collate_fn_mtl(batch: List[Dict], pad_token_id: int = 0, tokenizer=None, 
                   backbone_name: str = "whisper") -> Dict:
    """
    Custom collate function for MTL with preprocessed audio.
    """
    batch_size = len(batch)
    ...
    if tokenizer:
        ...
        asr_lengths = torch.tensor([len(ids) for ids in tokenized_ids], dtype=torch.long)
    else:
        asr_targets, asr_lengths = None, None
