...
class MTLEvaluator:
    ...
    def compute_asr_metrics(self, predictions, targets, words_batch):
        ...
        for pred, target, words in zip(predictions, targets, words_batch):
            ...
            target_text = " ".join([words[i]
                                   for i in target if i < len(words)])
    ...
    def flatten_and_filter_sequences(self, predictions, targets, max_length=None):
        ...
        for pred_seq, target_seq in zip(predictions, targets):
            ...
            seq_len = len(target_seq) if max_length is None else min(
                len(target_seq), max_length)
    ...
    def add_batch_predictions(self, batch_data, outputs):
        ...
        if 'asr_logits' in outputs and 'asr_targets' in batch_data:
            ...
            for i, (pred, target, words) in enumerate(zip(asr_preds, asr_targets, words_batch)):
                pred_text = " ".join(
                    [words[j] for j in pred.cpu().numpy() if j < len(words)])
                target_text = " ".join(
                    [words[j] for j in target.cpu().numpy() if j < len(words)])

                # Calculate individual WER/CER
                individual_wer = wer(
                    [target_text], [pred_text]) if target_text.strip() else 0.0
                individual_cer = cer(
                    [target_text], [pred_text]) if target_text.strip() else 0.0
                ...
                self.predictions['asr'].append({
                    ...
                    'wer': individual_wer,
                    'cer': individual_cer,
                    'target_ids': target.cpu().numpy().tolist(),
                    'predicted_ids': pred.cpu().numpy().tolist()
                })
        ...
        if 'prosody_logits' in outputs:
            ...
            for i, (pred, target, words) in enumerate(zip(prosody_preds, prosody_targets, words_batch)):
                ...
                valid_positions = target_np != 0  # Assuming 0 is padding
                if valid_positions.sum() > 0:
                    word_accuracy = (
                        pred_np[valid_positions] == target_np[valid_positions]).mean()
                else:
                    word_accuracy = 0.0
                ...
                self.predictions['prosody'].append({
                    ...
                    'num_words': len(words),
                    'num_prominent_true': int(target_np.sum()),
                    'num_prominent_pred': int(pred_np.sum())
                })
    ...
    def analyze_predictions(self):
        ...
        if self.predictions['asr']:
            ...
            analysis['asr'] = {
                ...
                'average_wer': np.mean([p['wer'] for p in asr_preds]),
                'average_cer': np.mean([p['cer'] for p in asr_preds]),
        ...
        if self.predictions['prosody']:
            ...
            analysis['prosody'] = {
                ...
                'average_word_accuracy': np.mean([p['word_accuracy'] for p in prosody_preds]),
                'prominence_precision': self._calculate_prominence_precision(prosody_preds),
                'prominence_recall': self._calculate_prominence_recall(prosody_preds),
        ...
        if self.predictions['emotion']:
            ...
            analysis['emotion'] = {
                ...
                'accuracy': np.mean([p['correct'] for p in emotion_preds]),
